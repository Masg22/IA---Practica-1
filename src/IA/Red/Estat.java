...
public class Estat {
	...
	// Class definition ConnexSensor
		...
		public class ConnexSensor {
			...
			//Cridada sobre el primer par�metre
			...
			//Cridada sobre el segon par�metre
			...
			public Boolean recActTransmission(Double transmissionChange, int actual){
				...
				while (! q.isEmpty()) {
					...
					if (connex < 0) { // CENTRE
						Boolean caux = connexCList.get(- connex).actCapacity(transmissionChange);
						if(!caux) {
			...
			public ArrayList<Integer> getConnectionIn() {
				return new ArrayList<Integer>(this.connectionIn);
			}
