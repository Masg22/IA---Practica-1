...
public class Dibuix extends Canvas {
	...
	/**
	 * Radi dels punts dibuixats. De cara a enfora, el radi es un enter pero
	 * internament es un double per a tenir mes precisio al calcular les fletxes
	 */
	...
	/**
	 * Determina la coordinada que correspon al limit esquerre del que es mostra per
	 * pantalla del dibuix
	 */
	private int _esquerre;
	/**
	 * Determina la coordinada que correspon al limit dret del que es mostra per
	 * pantalla del dibuix. Ha de ser superior al limit esquerre
	 */
	private int _dret;
	/**
	 * Determina la coordinada que correspon al limit superior del que es mostra per
	 * pantalla del dibuix
	 */
	private int _superior;
	/**
	 * Determina la coordinada que correspon al limit inferior del que es mostra per
	 * pantalla del dibuix. El seu valor ha de ser superior al valor de _superior
	 */
	...
	/**
	 * colors amb que es pinten els punts (cada punt es pot pintar d'un color
	 * diferent).
	 */
	private HashMap _colorPunts;
	/**
	 * colors amb que es pinten les fletxes (cada fletxa es pot pintar d'un color
	 * diferent).
	 */
	...
	/**
	 * Diferencia entre coordenades minima per a evitar problemes si hi ha massa
	 * zoom
	 */
	...
	public Dibuix() {
		/*
		 * //inicialitzar el component "Canvas" sobre el que dibuixem super ();
		 * //inicialitzar els vectors de punts i fletxes _posX = new HashMap(); _posY =
		 * new HashMap(); _etiqPunt = new HashMap(); _puntsOrigen = new Vector();
		 * _puntsDesti = new Vector(); //fons de color blanc setBackground
		 * (Color.white); //per defecte es mostren les etiquetes _mostrarEtiquetes =
		 * true; //inicialitzar les coordenades _esquerre=0; _dret=getSize().width; if
		 * (_dret < MINDIFERENCIA) { _dret = MINDIFERENCIA+1; } _superior=0;
		 * _inferior=getSize().height; if (_inferior < MINDIFERENCIA) { _inferior =
		 * MINDIFERENCIA+1; } //per defecte no hi ha cap punt seleccionat _seleccionat =
		 * -1; //calcular el radi dels punts del dibuix calcularRadi(); //colors per
		 * defecte _colorPunt = Color.black; _colorFletxa = Color.black;
		 * _colorSeleccionat = Color.green; //al principi no es dibuixa cap quadricula
		 * _dibuixarQuadricula = false; _colorQuadricula = Color.black;
		 * _tamanyQuadriculaX = 50; _tamanyQuadriculaY = 50;
		 */
		super();
	...
	/**
	 * Constructora de la classe en que s'especifica el tamany que ha de tenir
	 * 
	...
	/**
	 * Retorna el radi amb que es dibuixen els punts. Externament el radi es veu com
	 * un enter, tot i que per millorar la precisio internament es un double
	 * 
	...
	/**
	 * Retorna el color amb que es dibuixa la fletxa que va del punt origen al punt
	 * desti.
	 * 
	...
	/**
	 * Retorna si es mostren les etiquetes dels punts Encara que retorni cert, si el
	 * zoom no es suficient en el dibuix, es possible que les etiquetes no es
	 * dibuixin. Llavors s'ha d'apropar mes la imatge amb el zoom per veure les
	 * etiquetes.
	 * 
	...
	public int getPosicioX(int id) throws Exception {
		int res = 0;
		if (_posX.containsKey(Integer.valueOf(id))) {
			res = ((Integer) _posX.get(Integer.valueOf(id))).intValue();
		} else {
	...
	public int getPosicioY(int id) throws Exception {
		int res = 0;
		if (_posY.containsKey(Integer.valueOf(id))) {
			res = ((Integer) _posY.get(Integer.valueOf(id))).intValue();
		} else {
			throw new Exception("El punt " + id + " no existeix");
		}
		return res;
	 ...
	 * 
	 * @return el punt en l'eix de les x corresponent al limit esquerre del dibuix
	 */
	 ...
	 * 
	 * @return el punt en l'eix de les y corresponent al limit superior del dibuix
	 */
	 ...
	 * 
	 * @return el punt en l'eix de les y corresponent al limit inferior del dibuix
	 */
	 ...
	 * 
	 * @return la distancia entre linies verticals consecutives de la quadricula
	 */
	 ...
	 * 
	 * @return el desplacament inicial per si la quadricula no comenca al punt 0
	 */
	 ...
	 * 
	 * @return la distancia entre linies horitzontals consecutives de la quadricula
	 */
	...
	public String getEtiquetaPunt(int id) throws Exception {
		String res = "";
		if (_etiqPunt.containsKey(Integer.valueOf(id))) {
			res = (String) _etiqPunt.get(Integer.valueOf(id));
		} else {
			throw new Exception("El punt " + id + " no existeix");
		}
		return res;
	 ...
	 * identificador (un natural). Si hi ha mes d'un punt retorna el d'identificar
	 * menor. Sino retorna -1. En sistemes de coordinades amb molta diferencia entre
	 * amplada i altura de les coordenades, pot donar alguns problemes en els
	 * contorns del punt, ja que esta intentant reconeixer un punt quan
	 * conceptualment hi ha dibuixat un oval que es veu com un punt degut a la
	 * deformacio del sistema de coordenades. En qualsevol cas, el centre del punt
	 * sempre el reconeix.
	 * 
	...
	public int existeixPunt(int x, int y) {
		...
		double radix, radiy;
		/*
		 * si la imatge no te coordenades quadrades, al dibuixar-se el punt rodo, al
		 * mirar el diametre per les coordenades no sera el mateix el diametre
		 * horitzontal que el vertical
		 */
		...
		for (int k = _posX.size() - 1; k >= 0 && !trobat; k--) {
			...
			int x1 = ((Integer) _posX.get(Integer.valueOf(i))).intValue();
			x1 -= x;
			x1 *= x1;
			int y1 = ((Integer) _posY.get(Integer.valueOf(i))).intValue();
			y1 -= y;
			y1 *= y1;
	...
	/**
	 * Canvia el tamany del dibuix reestablint el sistema de coordenades
	 * 
	 * @param width  amplada del dibuix
	 * @param height altura del dibuix
	 */
	public void setSize(int width, int height) {
		...
		_superior = 0;
		/*
		 * si el tamany del component es massa petit, posem les coordenades minimes
		 */
		if (_dret < MINDIFERENCIA) {
			_dret = MINDIFERENCIA + 1;
		}
	 ...
	 * @throws Exception si r es menor o igual a 0 o es mes gran que l'amplada o
	 *                   altura del dibuix
	 */
	...
	public void setColorPunt(int id, Color c) throws Exception {
		if (!_colorPunts.containsKey(Integer.valueOf(id))) {
			throw new Exception("El punt no existeix");
		}
		_colorPunts.put(Integer.valueOf(id), c);
		repaint();
	}

	/**
	 * Estableix el color amb que es pinten les fletxes. Si hi ha fletxes
	 * bidireccionals de diferents colors, el tronc de la fletxa es veura del color
	 * que fa menys temps que ha estat modificat.
	 * 
	 ...
	 */
	public void setColorFletxa(int origen, int desti, Color c) throws Exception {
		boolean trobat = false;
		int lloc = 0;
		int i = 0;
		...
		if (trobat) {
			if (posterior) {
				/*
				 * intercanviem l'ordre de les fletxes perque el nou color sigui el que es vegi
				 */
				_puntsOrigen.set(lloc, Integer.valueOf(desti));
				_puntsOrigen.set(post, Integer.valueOf(origen));
				_puntsDesti.set(lloc, Integer.valueOf(origen));
				_puntsDesti.set(post, Integer.valueOf(desti));
	...
	/**
	 * Determina si s'han de mostrar o no les etiquetes dels punts. Tot i que es
	 * posi que s'han de mostrar, si el dibuix no s'apropa suficientment als punts
	 * les etiquetes no es mostraran, per intentar evitar solapaments entre punts i
	 * etiquetes i fletxes.
	 * 
	...
	/**
	 * Decideix si es dibuixa o no quadricula en el dibuix. En cas que s'hagi de
	 * dibuixar, comprova que els parametres son correctes i dibuixa
	 * 
	...
	/**
	 * Afegeix un nou punt en el dibuix en el lloc (x,y) amb etiqueta buida Retorna
	 * l'enter que identifica aquest punt dintre el dibuix
	 * 
	...
	public int dibuixarPunt(int x, int y) throws Exception {
		...
		/*
		 * int i = 0; while (_posX.containsKey(Integer.valueOf(i))) { i++; } //i es una
		 * clau buida _posX.put(Integer.valueOf(i),Integer.valueOf(x)); try {
		 * _posY.put(Integer.valueOf(i),Integer.valueOf(y)); } catch (Exception e) {
		 * //si no hem pogut entrar la segona coordenada, esborrem la primera
		 * _posX.remove(Integer.valueOf(i)); throw e; } //afegim una etiqueta buida al
		 * punt try { _etiqPunt.put(Integer.valueOf(i),new String("")); } catch
		 * (Exception e) { //si no podem entrar l'etiqueta, esborrem el fet fins ara
		 * _posX.remove (Integer.valueOf(i)); _posY.remove (Integer.valueOf(i)); throw
		 * e; } repaint(); //retorna l'identificador intern del punt return i;
		 */
	}

	/**
	 * Afegeix un nou punt en el dibuix en el lloc (x,y) amb etiqueta s Retorna
	 * l'enter que identifica aquest punt dintre el dibuix
	 * 
	 * @param x punt en l'eix de les x en que es posa el punt
	 * @param y punt en l'eix de les y en que es posa el punt
	...
	public int dibuixarPunt(int x, int y, String s) throws Exception {
		int i = 0;
		while (_posX.containsKey(Integer.valueOf(i))) {
			i++;
		}
		// i es una clau buida
		_posX.put(Integer.valueOf(i), Integer.valueOf(x));
		try {
			_posY.put(Integer.valueOf(i), Integer.valueOf(y));
		} catch (Exception e) {
			// si no hem pogut entrar la segona coordenada, esborrem la primera
			_posX.remove(Integer.valueOf(i));
			throw e;
		}
		// afegim una etiqueta buida al punt
		try {
			_etiqPunt.put(Integer.valueOf(i), s);
		} catch (Exception e) {
			// si no podem entrar l'etiqueta, esborrem el fet fins ara
			_posX.remove(Integer.valueOf(i));
			_posY.remove(Integer.valueOf(i));
			throw e;
		}
		// afegim el color del punt
		try {
			_colorPunts.put(Integer.valueOf(i), _colorPunt);
		} catch (Exception e) {
			// Si no hem pogut afegir el color esborrem el fet fins ara
			_posX.remove(Integer.valueOf(i));
			_posY.remove(Integer.valueOf(i));
			_etiqPunt.remove(Integer.valueOf(i));
			throw e;
		}
		repaint();
	...
	public void setEtiquetaPunt(int id, String s) throws Exception {
		if (_etiqPunt.containsKey(Integer.valueOf(id))) {
			_etiqPunt.put(Integer.valueOf(id), s);
		} else {
			throw new Exception("El punt " + id + " no existeix");
		}
		repaint();
	...
	public void mourePunt(int id, int x, int y) throws Exception {
		...
		int aux = ((Integer) _posX.get(Integer.valueOf(id))).intValue();
		_posX.put(Integer.valueOf(id), Integer.valueOf(x));
		try {
			_posY.put(Integer.valueOf(id), Integer.valueOf(y));
		} catch (Exception e) {
			/*
			 * si hi ha hagut un error a l'entrar la segona dada, hem de restaurar la
			 * primera
			 */
			_posX.put(Integer.valueOf(id), Integer.valueOf(aux));
	...
	public void suprimirPunt(int id) {
		...
		_posX.remove(Integer.valueOf(id));
		_posY.remove(Integer.valueOf(id));
		_colorPunts.remove(Integer.valueOf(id));
		_etiqPunt.remove(Integer.valueOf(id));
		repaint();
	}

	/**
	 * Afegeix una nova fletxa entre els punts origen i desti. No es permeten
	 * fletxes entre el mateix punt ni repetides
	 * 
	...
	public void dibuixarFletxa(int origen, int desti) throws Exception {
		boolean trobat = false;
		if (!_posX.containsKey(Integer.valueOf(origen)) || !_posY.containsKey(Integer.valueOf(desti))) {
		...
		if (!trobat && origen != desti) {
			_puntsOrigen.addElement(Integer.valueOf(origen));
			try {
				_puntsDesti.addElement(Integer.valueOf(desti));
			} catch (Exception e) {
				/*
				 * si no s'ha pogut afegir el punt de desti, esborrem el d'origen per mantenir
				 * la coherencia entre els vectors
				 */
			...
			try {
				...
				/*
				 * si no s'ha pogut afegir l'element, esborrem tot el que s'ha tocat fins ara
				 */
				_puntsOrigen.removeElementAt(_puntsOrigen.size() - 1);
	...
	/**
	 * Enquadra les coordenades d'acord amb els parametres. Posa el limit superior
	 * esquerre en (x1,y1) i l'inferior dret en (x2,y2). Només ho fa en cas que no
	 * inverteixin la imatge (x2>x1 i y2>y1) i la distancia entre punts sigui com a
	 * minim 10.
	 * 
	...
	public void enquadrar(int x1, int y1, int x2, int y2) throws Exception {
		if (x2 > x1 + MINDIFERENCIA && y2 > y1 + MINDIFERENCIA) {
			...
			/*
			 * les coordenades no son correctes. Si no es controles aquest error el dibuix
			 * podria sortir invertit o del reves, i es vol evitar aquest fet.
			 */
	...
	/**
	 * Canvia el sistema de coordenades ampliant la imatge el percentatge que indica
	 * el parametre rebut
	 * 
	...
	public void zoom(int percentatge) throws Exception {
		...
		if (!correcte) {
			/*
			 * tot i que no es cap error, es llanca una excepcio per indicar que mes zoom no
			 * modificara la imatge
			 */
	...
	/**
	 * Mou el sistema de coordenades tantes unitats com indiquen els parametres
	 * 
	...
	private void dibuixarPunts(Graphics g) {
		...
		for (int k = 0; k < _posX.size(); k++) {
			// calcul de la distancia entre el punt entrat i el punt i
			i = ((Integer) key[k]).intValue();
			g.setColor((Color) _colorPunts.get(Integer.valueOf(i)));
			if (i == _seleccionat) {
				g.setColor(_colorSeleccionat);
			}
			x1 = ((Integer) _posX.get(Integer.valueOf(i))).intValue() - _esquerre;
			y1 = ((Integer) _posY.get(Integer.valueOf(i))).intValue() - _superior;
			...
			s = (String) _etiqPunt.get(Integer.valueOf(i));
			/*
			 * nomes dibuixem l'etiqueta si estem prou aprop, per intentar evitar
			 * solapaments entre lletres i punts. Es podrien utilitzar algorismes per a
			 * evitar tots els solapaments, pero per eficiencia (seria molt farragos en cas
			 * de molts punts) millor no tocar-ho ja que ja hi ha un boolea per si no es vol
			 * mostrar els punts
			 */
			...
			/*
			 * if (i == _seleccionat) { g.setColor (_colorPunt); }
			 */
		}
	}
	...
	private void dibuixarFletxes(Graphics g) {
		...
		for (int i = 0; i < _puntsOrigen.size(); i++) {
			...
			/*
			 * cerca de les coordenades dels punts de la fletxa adequant les coordenades
			 */
			o = ((Integer) _puntsOrigen.elementAt(i)).intValue();
			x1 = ((Integer) _posX.get(Integer.valueOf(o))).intValue() - _esquerre;
			x1 = x1 * rang_x / (_dret - _esquerre);
			y1 = ((Integer) _posY.get(Integer.valueOf(o))).intValue() - _superior;
			y1 = y1 * rang_y / (_inferior - _superior);
			d = ((Integer) _puntsDesti.elementAt(i)).intValue();
			x2 = ((Integer) _posX.get(Integer.valueOf(d))).intValue() - _esquerre;
			x2 = x2 * rang_x / (_dret - _esquerre);
			y2 = ((Integer) _posY.get(Integer.valueOf(d))).intValue() - _superior;
			 ...
			 */
			/*
			 * pol_y[0]=y2; pol_x[0]=x2; pol_y[1]=pol_y[0]-
			 * 2*(int)(radiaux/2*Math.sin(angle-Math.PI/4)); pol_y[2]=pol_y[0]-
			 * 2*(int)(radiaux/2*Math.cos(angle-Math.PI/4)); if (x2>x1) { pol_x[1]=pol_x[0]-
			 * 2*(int)(radiaux/2*Math.cos(angle-Math.PI/4));
			 * 
			 * pol_x[2]=pol_x[0]+ 2*(int)(radiaux/2*Math.sin(angle-Math.PI/4)); } else {
			 * pol_x[1]=pol_x[0]+ 2*(int)(radiaux/2*Math.cos(angle-Math.PI/4));
			 * pol_x[2]=pol_x[0]- 2*(int)(radiaux/2*Math.sin(angle-Math.PI/4)); } //dibuixar
			 * la fletxa g.fillPolygon (pol_x,pol_y,3);
			 */
		}
	}
	...
	private void dibuixarQuadricula(Graphics g) {
		...
		int i = (_iniciQuadriculaX - _esquerre) % _tamanyQuadriculaX;
		/*
		 * des del principi de la part del dibuix que es mostra, anar fent una linia
		 * vertical cada _tamanyQuadriculaX. S'ha de traduir les coordenades de
		 * (_esquerre a _dreta) a (0 a width)
		 */
