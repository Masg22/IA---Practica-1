...
public class ConnectatBoard {
	...
	public void generarEstatInicial(int tipus) {
		...
		case 3:
			...
			posx = ((node) centr.elementAt(0)).getX();
			posy = ((node) centr.elementAt(0)).getY();
			centr.removeElementAt(0);
			...
			while (centr.size() != 0) {
				...
				if (centr.size() != 0) {
					// buscar la central més propera
					posmin = 0;
					x = ((node) centr.elementAt(0)).getX();
					y = ((node) centr.elementAt(0)).getY();
					valor = Math.abs(x - posx) + Math.abs(y - posy);
		...
		case 4:
			...
			posx = ((node) centrep.elementAt(0)).getX();
			posy = ((node) centrep.elementAt(0)).getY();
	 ...
	 * 
	 * connexions[a][b]=false; connexions[b][a]=false;
	 * 
	 * connexions[b][c]=true; connexions[c][b]=true;
	 * 
	 * calcularGraus(); error_total=calcularError(); }
	 */
